...
public class server1 {
    private static byte[] BytetoHeader(int numofseq, int numofack, int ack, int sf, int rwnd) {
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        try {
            outputStream.write(ByteBuffer.allocate(4).putInt(numofseq).array());
            outputStream.write(ByteBuffer.allocate(4).putInt(numofack).array());
    ...
    private static int[] GetfromHeader(byte[] segment) {
        ...
        try {
            ...
            while (true) {
                ...
                if (seqNum == expectedSeqNum) {
                    ...
                    byte[] toSendAck = BytetoHeader(seqNum, ackNum, 1, 0, 12);
